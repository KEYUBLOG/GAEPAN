...
async function callGemini(req: JudgeRequest): Promise<JudgeVerdict> {
  ...
  const systemInstruction = [
    ...
    "- 사용자 메시지에 [카테고리: 연애/친구/가족/직장생활/결혼생활/육아/이웃/학교/군대/기타]가 있거나, 사건 내용이 진짜 형사사건이 아니라 개인 간 갈등·고민·논쟁·일상 다툼(연인·친구·가족·직장 갈등, 누가 잘못했는지 논쟁, 예의·배려 문제)이면, 이 규칙이 다른 모든 규칙보다 우선한다. 징역·벌금·사회봉사는 절대 선고하지 말고 반드시 유머러스한 판결만 하라.",
    "- 이 경우 rationale은 짧게, verdict 주문은 반드시 '야식 금지 1주', '설거지 3회 실시', '커피 한 잔 쏘기', '진심 어린 사과 1회', 'SNS 24시간 일시 정지', '반성문 500자 작성', '손절 1회', '대화 10분 듣기' 등 가벼운 유머러스 주문으로만 끝내라. 법정형·징역·벌금 언급 금지.",
    "",
    ...
    "",
    "법리 적용 원칙:",
    "1. 판례 기반 분석: 쟁점과 유사한 대한민국 대법원·하급심 판례를 참조하고, '대법원 20XX. X. X. 선고 20XX도XXXX 판결' 형식으로 인용한 뒤, 해당 법리가 본 사건에 어떻게 적용되는지 논증하라. 인용한 판례의 죄명·형량이 본건 선고와 어긋나지 않도록 하라.",
  ...
  const categoryHint = isHumorousCategory
    ? `【최우선 지시】 카테고리: ${req.category}. 이 사건은 개인 간 갈등·고민·논쟁입니다. 실제 살인·상해·사기·강도가 아니므로 반드시 유머러스한 판결만 하라. 징역·벌금·사회봉사는 절대 쓰지 말고, 주문은 '야식 금지', '설거지 3회', '커피 쏘기', '사과 1회' 등 가벼운 형태로만 선고하라.`
    : "";
  ...
  const userMessage = [
    ...
    `피고인: ${req.defendant}`,
    ...(isHumorousCategory ? ["[유의] 위 카테고리이므로 유머러스한 주문만 선고하라. 징역·벌금 금지.", ""] : []),
